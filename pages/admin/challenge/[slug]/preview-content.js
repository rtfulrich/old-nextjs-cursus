...
export default function PreviewChallengeContent({ content }) {
...
export async function getServerSideProps({ params, req }) {
	return await getPageProps(async () => {
		const response = await axios.get(`${ADMIN_API_URL}/challenge/${params.slug}?challenge-data-only`, {
			headers: {
				credentials: "include",
				referer: FRONT_URL,
		...
		const { content, title } = response.data.challengeData;

		return {
			props: {
				page: {
					title: `Preview challenge content - ${title}`
				},
				content,
			}
		}
