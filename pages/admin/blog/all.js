...
export default function ViewAllBlogs({ result }) {
							...
							<div className="my-2 px-2 flex-1 flex flex-col justify-between">
								<h1 className="font-bold mb-2">
									<Link href={`/admin/blog/${blog.id}/${blog.slug}`}>
										<a className="hover:text-blue-500" style={{ textDecoration: "none" }}>{blog.title}</a>
									</Link>
								</h1>
...
export async function getServerSideProps({ req, query }) {
	try {
		...
		const response = await axios.get(`${ADMIN_API_URL}/blogs${page ? `?page=${page}` : ""}`, {
		...
		return {
			props: {
				page: {
					title: `All blogs${page ? ` - page ${page}` : ""}`
				},
				result
			}
		};
